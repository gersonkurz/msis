...
func main() {
	...
	for _, filename := range args.files {
		...
		if artifactOnlySBOM(args) {
			...
			if err == nil {
				err = runSBOM(filename, args.analyze)
			}
			if err != nil {
...
func processMSIFile(setup *ir.Setup, vars variables.Dictionary, workDir, templateFolder, customTemplates, filename string, args *cliArgs) error {
	...
	if args.build {
		...
		if args.sbom {
			rec.Sort()
			return emitBuildSBOM(rec, []string{msiPath}, supplied, declared, statements, args.scan, args.scanDir, args.analyze)
		}
	}
...
func processAutoBundle(setup *ir.Setup, vars variables.Dictionary, workDir, templateFolder, customTemplates, msiPath string, args *cliArgs, rec *buildrecord.Record, supplied []sbom.Supplied, declared []sbom.Declaration,
	...
	if args.sbom {
		...
		rec.Sort()
		return emitBuildSBOM(rec, []string{msiPath, bundleBuilder.OutputFile}, supplied, declared, statements, args.scan, args.scanDir, args.analyze)
	}
...
func processBundleFile(setup *ir.Setup, vars variables.Dictionary, workDir, templateFolder, customTemplates, filename string, args *cliArgs) error {
	...
	if args.build {
		...
		if args.sbom {
			rec.Sort()
			return emitBuildSBOM(rec, []string{builder.OutputFile}, nil, nil, statements, args.scan, args.scanDir, args.analyze)
		}
	}
...
func parseArgs() *cliArgs {
	...
	fs.BoolVar(&args.analyze, "analyze", false, "")
...
func printUsage() {
	...
	fmt.Printf("  %s            Report what is inside a built .msi or bundle .exe\n", cli.Info("/INSPECT"))
	...
	fmt.Printf("  %s            With /SBOM: run syft on the MSI's payload and add the packages\n", cli.Info("/ANALYZE"))
	fmt.Printf("  %s                that declare themselves (dist-info METADATA, pom.properties,\n", cli.Info("     "))
	fmt.Printf("  %s                .deps.json), each attached to the file it was found in\n", cli.Info("     "))
